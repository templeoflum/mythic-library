...
| **Codex** | Browse catalog items. Sub-tabs: **Archetypes** (996 with 8D coordinates, primordials, relationships), **Entities** (159 mapped from Library, 14 unmapped places/concepts), **Motifs** (149 Thompson Index codes with pattern cross-refs). |
...
│   ├── archetypes_catalog.json    # 996 archetypes with coordinates, primordials, relationships
...
│   └── archetype_affinities.json  # Archetype-to-node affinity scores (legacy)
├── css/
...
- ACP JSON-LD files (996 archetypes across 22 primordials)
...
─────────────          ──────────────           ──────────────
173 entities    →→→    996 archetypes    →→→    19 nodes
